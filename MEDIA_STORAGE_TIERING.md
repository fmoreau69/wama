...
| **2** | médiathèque → app | *« les fichiers restent dans la médiathèque, pas d'import dans app/input, tu confirmes ? »* | ❌ **FAUX pour le geste courant** : le bouton « Médiathèque » d'une card fait `fetch(file_url)` → `blob` → `new File` → **re-upload** = COPIE autonome dans `app/input`, **sans aucun lien** avec l'asset. Double transfert, double stockage. Sur 5 consommateurs de `MediaPicker`, **1 seul** (Studio) lit le 2ᵉ argument `asset` — et subit quand même le téléchargement du blob, fait AVANT `onSelect`. ✅ **VRAI pour 2 voies serveur** : nœud Studio `media_import` (`asset_path`) et voix `ua_<id>` du synthesizer, lus EN PLACE | `common/static/common/js/media-picker.js:177-192` ; `_new_item_card.html:119` ; `studio/tasks.py:57-68` ; `common/tts/voice_refs.py:188-196` |
...
| **D11** | **`WamaImport` n'est chargé par AUCUNE des 10 apps en place** — `_app_scripts.html` n'est inclus que par les gabarits GÉNÉRÉS (mesuré 05/09 sur imager, avatarizer, converter : `typeof WamaImport === 'undefined'`). La « voie d'import commune » est donc, aujourd'hui, la voie des seules jumelles ; chaque app en place garde son `handleFiles` | portage | ✅ **10/10 le 08/09 — portage TERMINÉ** (+ composer : mode ATTACHE déclaratif ; + enhancer-AUDIO : barre de lot et logique communes, `idBase` ; + imager ×2 domaines et avatarizer : attache, dont « sur soi-même » ; + enhancer-image : URL par le formalisme de lot). Plus aucune boucle d'import propre à une app, `import_front` 10/10 ; **lot VIDÉO à l'imager** (le domaine du lot est déclaré, `import_batch` le lit — le studio y postera) — détail `WAMA_APP_GENERATION_ROUTE §Portage F2`. État du 07/09 : **7/10 : transcriber (13/13), converter (13/14 + skip anti-bouclage), describer (12/12), synthesizer (12/13 + skip « pas d'URL »), enhancer-image (11/12 + skip anti-bouclage), reader (11/12 + skip « pas d'URL »), anonymizer (**12/12 le 08/09** : import par URL porté sur le formalisme de lot commun, régime paresseux comme les autres — plus aucun `$.ajax` maison ni modale) câblés**, critère `import_front` écrit — détail `WAMA_APP_GENERATION_ROUTE §Portage F2` (dont le 500 de consolidation sur FormData corrigé dans 6 vues, et l'évolution 8 « progression d'envoi ») ; restent composer (vue `upload` à créer), enhancer-audio (lot maison), imager/avatarizer (attache = card v4) ; **tout helper que l'explorateur (global) doit atteindre va dans `wama-app-base.js`, jamais dans `wama-import.js`** | `common/_app_scripts.html:47` |
