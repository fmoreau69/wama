...
| R21 | **Barres de recherche/filtrage écrites à la main** | `licenses.html` (`#li-q` + `filtrer()`), `journal.html` (ma 4ᵉ variante, à boutons), `model_manager/index.html` (`applyFilters()`, ~50 l.) — et **2 pages sans aucune barre** (`apps.html`, `library_catalog.html`) | implémentations concurrentes du même geste, comportements divergents (avec/sans bouton, avec/sans compteur) | 🟡 EN COURS (2026-08-20) : brique `wama-filter-bar.js` + `_filter_bar.html` (apparence **model manager**, référence retenue par Fabien ; filtrage EN DIRECT ; options dérivées du DOM en client, déclarées en server). **journal** ✅ · **licences** ✅ *(empreinte 8 combinaisons identique avant/après)* · **function_catalog** ✅ **apparence seule** (cf. note) · restent **apps** + **library_catalog** (à doter) et **model_manager** (EN DERNIER, cf. note). ⚠ **Règle dégagée** : DÉCLARER les options prime sur les DÉRIVER dès que le libellé porte du sens (licences affichait « Modèles », la dérivation aurait rendu « model »). | ⛔ |
