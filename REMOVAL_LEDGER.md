...
| R21 | **Barres de recherche/filtrage écrites à la main** | `model_manager/index.html` (`applyFilters()`, ~50 l. inline + 4 selects), `function_catalog.html` (`#fcSearch`), `licenses.html` (`#li-q` + `filtrer()`), `journal.html` (ma 4ᵉ variante, à boutons) — et **2 pages qui n'en avaient aucune** (`apps.html`, `library_catalog.html`) | 3 implémentations du même geste + 1 ajoutée par moi ; comportements divergents (avec/sans bouton, avec/sans compteur) | 🟡 EN COURS (2026-08-20) : brique commune `wama-filter-bar.js` + `_filter_bar.html` (apparence **model manager**, retenue par Fabien comme référence ; filtrage EN DIRECT, options dérivées du DOM en mode client, déclarées en mode server pour les listes paginées). **journal PORTÉ** ✅. Restent : model_manager, function_catalog, licences (à retirer), apps + library_catalog (à doter) | ⛔ |
