...
| R20 | `wama-inspector.js` + `wama-inspector-autofill.js` chargés **localement** | 13 gabarits (10 apps + `common/apps.html` + `model_manager` + `studio`) — **19 balises** | redondant : `base.html` fournissait déjà leur CSS et leurs hôtes DOM (`#inspectorInfo`, `#info-section`), mais pas les scripts ; une page transversale (journal) ne les avait donc pas du tout | ✅ SOLDÉ (2026-08-20) : montés globalement dans `base.html`, 19 inclusions locales retirées **dans le même commit** + commentaires anti-ré-inclusion. ⚠ **Préalable indispensable** : le `MutationObserver` de `wama-inspector.js` re-scannait TOUT le document à chaque ajout de nœuds — inoffensif sur 13 pages à cards, coûteux une fois global (filemanager/jstree, polling). Filtré sur les nœuds ajoutés AVANT de globaliser (0,5 ms médiane / 300 nœuds mesuré). Vérifié : exactement 1 occurrence sur 5 pages | ✅ |
