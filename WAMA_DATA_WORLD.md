...
- **C** : `group` (rôle) sur `PortSpec` — vocabulaire emprunté à `INPUT_TYPES`, jamais un 3ᵉ enum ; requalifier les références de fait (`track`/`road_map` `optional=True` du cam_analyzer) ; accesseur `function_node_ports()` rendant la MÊME forme que `studio_node_ports` → la card v4 couvre Data sans une ligne par consommateur ;
- **D** : le nœud `function` du kind pipeline (D13) — dispatch dans l'EXÉCUTEUR (app = job de file async, function = transformation typée sync), jamais dans le schéma ;
