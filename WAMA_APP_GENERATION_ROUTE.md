...
| 22 | **`batch_create` encore bouchonné (501)** : les pièces existent au commun (`batch_common.group_into_batches_by_nature`, `wrap_in_batch`) et l'app en place les assemble (`converter/views.py::consolidate_jobs_into_batches`). Comme pour `batch_preview` et `consolidate`, ce n'est pas une politique d'app — c'est du câblage. Sans lui, un fichier de lot s'aperçoit mais n'aboutit pas. | F1 | codegen |
