...
| 20 | **Routes par-outil `/api/tools/*` NON gardées par le gating F7** (trouvé 2026-08-13 en traçant l'API) : les 14 vues individuelles de `wama/tool_api.py` (« for manual testing ») appellent les fonctions d'outil DIRECTEMENT — sans `execute_tool`, donc sans `tool_accessible` — et `app_id_for_path('/api/tools/…')` → None (segment `api`) : le middleware ne les couvre pas non plus. Seul `@login_required` s'applique : tier + rôles contournés. **Jumeau exact du trou #7** (même mécanique, autre surface — #7 n'avait couvert que `/api/v1/`). Deux issues : router ces vues par `execute_tool` (une seule porte), ou les déprécier au profit de la passerelle v1 (elles la précèdent historiquement) | F7 | sécurité/cohérence |
