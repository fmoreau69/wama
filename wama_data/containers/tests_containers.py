...
from . import (Context, Report, SCHEMAS, write, writable_extensions, schema_modules,
               schema_for, available_schemas)


def _signal(name, temps, rows=None, *, data_type=DataType.TIMESERIES, ends=None, **kw):
    meta = SignalMeta(name=name, data_type=data_type, **kw)
    acces = (lambda i0, i1: rows[i0:i1]) if rows is not None else None
...
class RegistreTest(unittest.TestCase):
    ...
    def test_les_deux_schemas_livres_sont_enregistres(self):
        self.assertEqual(available_schemas(), ['trip', 'wdat'])
    ...
    def test_un_schema_se_trouve_par_son_nom_ou_par_une_extension(self):
        self.assertIs(schema_for('wdat'), SCHEMAS['wdat'])
        self.assertIs(schema_for('/tmp/x/essai.trip'), SCHEMAS['trip'])
        self.assertIs(schema_for(Path('a.wdat')), SCHEMAS['wdat'])
        self.assertIsNone(schema_for('essai.inconnu'))

    def test_les_extensions_ecrivables_sont_annoncees(self):
        self.assertEqual(writable_extensions(), ['.trip', '.wdat'])
...
class EcritureTest(unittest.TestCase):
    ...
    def test_le_format_se_deduit_de_l_extension(self):
        rapport = write(self.ref, self.dossier / 'a.wdat')
    ...
    def test_une_extension_inconnue_est_refusee_en_nommant_les_formats_connus(self):
        with self.assertRaises(ValueError) as ctx:
            write(self.ref, self.dossier / 'a.inconnu')
    ...
    def test_un_conteneur_existant_n_est_PAS_ecrase_sans_le_dire(self):
        ...
        write(self.ref, cible)
        with self.assertRaises(FileExistsError):
            write(self.ref, cible)
        write(self.ref, cible, ecraser=True)   # explicite → autorisé
    ...
    def test_un_echec_ne_detruit_pas_la_version_precedente(self):
        ...
        with self.assertRaises(RuntimeError):
            write(casse, cible, ecraser=True)
    ...
    def test_on_peut_n_ecrire_qu_une_partie_des_flux(self):
        rapport = write(self.ref, self.dossier / 'a.wdat', stream=['vitesse'])
        self.assertEqual(list(rapport.tables), ['flux_vitesse'])
        self.assertEqual(rapport.rows, 3)
    ...
    def test_une_valeur_ABSENTE_arrive_en_NULL(self):
        ...
        `missing()` ne le fait pas échouer, parce que **SQLite coerce lui-même `NaN` en `NULL`**.
        ...
        values = [r[0] for r in con.execute('SELECT "v" FROM "flux_x" ORDER BY "time"')]
        con.close()
    ...
    def test_une_absence_DANS_une_structure_reste_du_JSON_VALIDE(self):
        ...
        spécification JSON n'accepte pas. Ni `missing()` ni SQLite ne couvraient ce cas."""
        import math
        ref = TemporalReferential()
    ...
    def test_une_colonne_apparue_APRES_la_premiere_tranche_est_SIGNALEE(self):
        ref = TemporalReferential()
        rows = [{'a': 1}, {'a': 2}, {'a': 3, 'surprise': 9}]
        ref.add(_signal('x', [0.0, 1.0, 2.0], rows))
        rapport = write(ref, self.dossier / 'a.wdat', tranche=2)
        self.assertTrue(any('surprise' in p for p in rapport.losses),
...
class WdatTest(unittest.TestCase):
    ...
    def test_le_nom_de_table_ne_dit_PAS_la_famille(self):
        ...
        rapport = write(self.ref, self.cible)
    ...
    def test_les_PERTES_d_acquisition_sont_une_colonne(self):
        ref = TemporalReferential()
        ref.add(_signal('x', [0.0], [{'a': 1}], losses=3))
        write(ref, self.cible)
    ...
    def test_la_copie_projetee_est_embarquee_et_estampillee(self):
        contexte = Context(auteur='fabien', manifestes=[
            {'manifest_kind': 'pipeline', 'key': 'protocole-a', 'schema_version': '2',
             'body': {'nodes': []}}])
        write(self.ref, self.cible, contexte=contexte)
        rows = self._table('SELECT manifest_kind, key, version, read_only, body '
                             'FROM "WamaManifests"')
        self.assertEqual(len(rows), 1)
        kind, key, version, lecture_seule, corps = rows[0]
        self.assertEqual((kind, key, version, lecture_seule), ('pipeline', 'protocole-a', '2', 1))
    ...
    def test_une_copie_projetee_SANS_ESTAMPILLE_est_REFUSEE(self):
        ...
        contexte = Context(manifestes=[{'body': {'nodes': []}}])
        with self.assertRaises(ValueError) as ctx:
            write(self.ref, self.cible, contexte=contexte)
    ...
    def test_l_auteur_et_la_date_sont_ecrits(self):
        ...
        write(self.ref, self.cible, contexte=Context(auteur='fabien', horodatage='2026-08-24'))
    ...
    def test_le_format_natif_ne_declare_AUCUNE_perte(self):
        rapport = write(self.ref, self.cible)
        self.assertTrue(rapport.fidele, rapport.losses)
...
class TripCompatibiliteTest(unittest.TestCase):
    ...
    def test_le_LECTEUR_EXISTANT_retrouve_les_flux_les_instants_et_les_valeurs(self):
        ...
        rows = par_nom['vitesse'].rows(0, 3)
        self.assertEqual([l['value'] for l in rows], [10.0, 11.5, None])
        self.assertEqual([l['mode'] for l in rows], ['auto', 'auto', 'manuel'])
    ...
    def test_la_colonne_de_TEMPS_figure_parmi_les_variables_comme_dans_le_releve(self):
        ...
        con.close()
        self.assertIn('startTimecode', naming)
        self.assertIn('endTimecode', naming)
    ...
    def test_un_segment_OUVERT_est_declare_PERDU(self):
        rapport = write(self.ref, self.cible)
        self.assertTrue(any('OUVERT' in p for p in rapport.losses), rapport.losses)

    def test_une_copie_projetee_est_declaree_NON_embarquee(self):
        contexte = Context(manifestes=[{'manifest_kind': 'pipeline', 'key': 'p'}])
        rapport = write(self.ref, self.cible, contexte=contexte)
        self.assertTrue(any('autoportant' in p for p in rapport.losses), rapport.losses)

    def test_les_pertes_d_acquisition_sont_declarees_PERDUES(self):
        ref = TemporalReferential()
        ref.add(_signal('x', [0.0], [{'a': 1}], losses=3))
        rapport = write(ref, self.cible)
        self.assertTrue(any("pertes d'acquisition" in p for p in rapport.losses), rapport.losses)

    def test_une_cadence_FRACTIONNAIRE_est_declaree_arrondie(self):
        ref = TemporalReferential()
        ref.add(_signal('x', [0.0], [{'a': 1}], fs=18.7))
        rapport = write(ref, self.cible)
        self.assertTrue(any('ARRONDIE' in p for p in rapport.losses), rapport.losses)
    ...
    def test_une_cadence_sur_un_EVENEMENT_est_declaree_perdue(self):
        ...
        rapport = write(ref, self.cible)
        self.assertTrue(any('frequency' in p for p in rapport.losses), rapport.losses)
    ...
    def test_une_famille_NON_DECLAREE_est_devinee_ET_signalee(self):
        ...
        self.assertTrue(any('DEVINÉE' in p for p in rapport.losses), rapport.losses)
...
class EncodageLecteurTest(unittest.TestCase):
    ...
    def test_le_lecteur_DECODE_le_texte_au_lieu_d_echouer(self):
        ...
        stream = lecteur.read(self.cible)
        self.assertEqual(stream[0].rows(0, 1)[0]['note'], 'Ajouté',
    ...
    def test_l_UTF8_reste_lu_comme_de_l_UTF8(self):
        ...
        con.commit()
        con.close()
        stream = lecteur_trip.TripReader().read(self.cible)
        self.assertEqual(stream[0].rows(0, 1)[0]['note'], 'Ajouté')


class RapportTest(unittest.TestCase):
    def test_un_rapport_sans_perte_est_fidele(self):
        self.assertTrue(Report(path='a', format='wdat', tables={'t': 2}).fidele)

    def test_un_rapport_compte_toutes_ses_lignes(self):
        self.assertEqual(Report(path='a', format='wdat',
                                 tables={'a': 2, 'b': 3}).rows, 5)
...
class WdatAllerRetourTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.cible = self.dossier / 'essai.wdat'
        self.ref = _referentiel()
        self.contexte = Context(auteur='fabien', horodatage='2026-08-24', manifestes=[
            {'manifest_kind': 'pipeline', 'key': 'protocole-a', 'schema_version': '2',
             'body': {'nodes': [{'id': 'n1'}]}}])
    ...
    def _relire(self):
        write(self.ref, self.cible, contexte=self.contexte)
    ...
    def test_les_PERTES_d_acquisition_reviennent(self):
        ...
        self.assertEqual(relu[0].meta.losses, 3)
...
class NomAbandonneTest(unittest.TestCase):
    ...
    def _occurrences(self):
        ...
        for f in sorted(monde.rglob('*.py')):
            ...
            if self.ABANDONNE in text.lower():
                rel = f.relative_to(monde).as_posix()
                trouve[rel] = [i for i, l in enumerate(text.splitlines(), 1)
                               if self.ABANDONNE in l.lower()]
        return trouve

    def test_le_nom_ABANDONNE_ne_survit_nulle_part_dans_le_monde(self):
        fautifs = [f"{rel}:{','.join(map(str, rows))}"
                   for rel, rows in self._occurrences().items()
    ...
    def test_le_nom_RETENU_est_bien_celui_qui_est_enregistre(self):
        ...
        self.assertIn('.wdat', writable_extensions())
