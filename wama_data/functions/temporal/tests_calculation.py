...
class NommageTest(unittest.TestCase):
    ...
    def test_la_colonne_produite_se_deduit_des_parametres(self):
        from .calculation import calcul_glissant, nom_produit
        self.assertEqual(nom_produit('vitesse', 'moyenne'), 'vitesse_moyenne')
        sortie = calcul_glissant(_signal(), fenetre_s=2.0, statistique='max')
    ...
    def test_deux_statistiques_ne_s_ecrasent_pas(self):
        ...
        from .calculation import calcul_glissant
        une = calcul_glissant(_signal(), 2.0, 'moyenne')
        deux = calcul_glissant(une, 2.0, 'max')
        self.assertIn('value_moyenne', deux.df.columns)
...
class EnrichissementTest(unittest.TestCase):
    ...
    def test_l_entree_n_est_JAMAIS_mutee(self):
        ...
        from .calculation import calcul_glissant
        signal = _signal()
        avant = list(signal.df.columns)
        calcul_glissant(signal, 2.0, 'moyenne')
        self.assertEqual(list(signal.df.columns), avant)

    def test_le_type_et_les_lignes_sont_conserves(self):
        from .calculation import calcul_derivee
        signal = _signal()
        sortie = calcul_derivee(signal)
    ...
    def test_une_sortie_enrichie_se_rechaine(self):
        from .calculation import calcul_derivee, calcul_glissant
        sortie = calcul_derivee(calcul_glissant(_signal(), 2.0, 'moyenne'),
                                colonne='value_moyenne')
        self.assertIn('value_moyenne_derivee', sortie.df.columns)

    def test_une_colonne_absente_est_signalee_et_NOMME_les_disponibles(self):
        from .calculation import calcul_glissant
        with self.assertRaises(ValueError) as e:
            calcul_glissant(_signal(), 2.0, 'moyenne', colonne='inexistante')
...
class IndicateursParSegmentTest(unittest.TestCase):
    ...
    def test_les_indicateurs_sont_ADJOINTS_aux_segments(self):
        from .calculation import calcul_par_segment
        segments = _segments([{'start': 0.0, 'end': 1.0, 'name': 'a'}])
        sortie = calcul_par_segment(segments, _signal(), statistiques='moyenne')
        ligne = sortie.df.to_dict('records')[0]
        self.assertEqual(ligne['name'], 'a', "les champs du segment survivent")
        self.assertEqual(ligne['value_moyenne'], 15.0)
    ...
    def test_les_indicateurs_sont_PREFIXES_du_nom_du_signal_mesure(self):
        ...
        from .calculation import calcul_par_segment
        sortie = calcul_par_segment(_segments([{'start': 0.0, 'end': 4.0}]), _signal(),
                                    statistiques='moyenne,max')
        for attendu in ('value_moyenne', 'value_max'):
    ...
    def test_les_champs_de_service_ne_sont_PAS_prefixes(self):
        ...
        from .calculation import calcul_par_segment
        sortie = calcul_par_segment(_segments([{'start': 0.0, 'end': 1.0}]), _signal())
    ...
    def test_un_segment_OUVERT_traverse_l_aller_retour_sans_se_refermer(self):
        ...
        self.assertTrue(lignes[1]['tronque'])
        ...
        self.assertTrue(manquant(lignes[1]['duree']))
        self.assertEqual(lignes[0]['duree'], 1.0)

    def test_un_segment_sans_mesure_donne_une_absence_et_non_un_zero(self):
        from .calculation import calcul_par_segment
        sortie = calcul_par_segment(_segments([{'start': 90.0, 'end': 99.0}]), _signal(),
                                    statistiques='moyenne')
        ligne = sortie.df.to_dict('records')[0]
        self.assertEqual(ligne['n'], 0)
        self.assertTrue(manquant(ligne['value_moyenne']), "pas de mesure ≠ mesure nulle")
        self.assertNotEqual(ligne['value_moyenne'], 0, "surtout pas un zéro crédible")
    ...
    def test_la_liste_de_statistiques_se_lit_separee_par_des_virgules(self):
        ...
        from .calculation import calcul_par_segment
        sortie = calcul_par_segment(_segments([{'start': 0.0, 'end': 4.0}]), _signal(),
                                    statistiques=' moyenne , min ')
        self.assertIn('value_moyenne', sortie.df.columns)
...
class CatalogueTest(unittest.TestCase):
    ...
    CLES = ('calcul_glissant', 'calcul_derivee', 'calcul_cumul', 'calcul_par_segment')
    ...
    def test_les_deux_modes_portent_des_categories_DIFFERENTES(self):
        ...
        self.assertEqual(get('calcul_glissant').category, 'enricher')
        self.assertEqual(get('calcul_par_segment').category, 'aggregate')

    def test_un_segmenteur_alimente_le_calcul_par_segment(self):
        sortie = get('segment_conditionnel').outputs[0]
        entree = get('calcul_par_segment').inputs[0]
    ...
    def test_un_signal_enrichi_alimente_le_calcul_par_segment(self):
        ok, raison = can_connect(get('calcul_glissant').outputs[0],
                                 get('calcul_par_segment').inputs[1],
    ...
    def test_toute_statistique_du_coeur_est_offerte_dans_la_modale(self):
        ...
        choix = [p for p in get('calcul_glissant').params if p.key == 'statistique'][0].choices
