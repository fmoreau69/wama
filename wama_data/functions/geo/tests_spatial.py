...
from ...core.segmentation import conditional, hysteresis_mask
from .spatial import distance_to_point
...
class DistanceAPointTest(unittest.TestCase):
    ...
    def test_la_colonne_est_ADJOINTE_et_nommee_du_POINT(self):
        out = distance_to_point(_trace([0.0, LOIN]), *CARREFOUR, name='carrefour_nord')
    ...
    def test_les_distances_sont_justes(self):
        out = distance_to_point(_trace([0.0, LOIN]), *CARREFOUR, name='c')
    ...
    def test_le_nom_est_NORMALISE_par_la_brique_commune(self):
        out = distance_to_point(_trace([0.0]), *CARREFOUR, name='Carrefour Nord (RD1)')
    ...
    def test_un_point_SANS_NOM_est_refuse(self):
        ...
        with self.assertRaises(ValueError) as ctx:
            distance_to_point(_trace([0.0]), *CARREFOUR)
        self.assertIn('nommer', str(ctx.exception))

    def test_deux_points_donnent_deux_colonnes(self):
        out = distance_to_point(_trace([0.0, LOIN]), *CARREFOUR, name='nord')
        out = distance_to_point(out, 45.76, 4.86, name='sud')
    ...
    def test_l_entree_n_est_PAS_mutee(self):
        t = _trace([0.0])
        distance_to_point(t, *CARREFOUR, name='c')
...
class CasCamAnalyzerTest(unittest.TestCase):
    ...
    def test_zone_a_rayon_reproduite_sans_code_dedie(self):
        ...
        avec = distance_to_point(trace, *CARREFOUR, name='carrefour')
        d = list(avec.df['distance_carrefour'])
        masque = [x is not None and x <= 40.0 for x in d]
        segs = conditional(list(avec.df['time']), masque)
    ...
    def test_le_GPS_qui_TREMBLE_sur_la_frontiere_ne_coupe_PAS(self):
        ...
        seuil_simple = conditional(temps, [x <= 39.0 for x in d])
        avec_hyst = conditional(temps, hysteresis_mask(d, 39.0, 60.0))
...
class MargesSpatialesWrapperTest(unittest.TestCase):
    ...
    def test_la_marge_en_metres_recule_la_borne_le_long_de_la_trace(self):
        ...
        self.assertEqual(s.df.iloc[0]['origin'], 'spatial_margins')
    ...
    def test_les_deux_marges_sont_DECLAREES_au_catalogue(self):
        ...
        for key in ('segment_margins', 'segment_spatial_margins'):
            spec = get(key)
            self.assertIsNotNone(spec, f"{key} absent du catalogue")
