...
from .frames import (adjoin, frame_from_referential, frame_from_signal,
                     signal_from_frame, default_type)


def _signal(name='vitesse', times=(0.0, 1.0, 2.0, 3.0), rows=None, ends=None, **kw):
    ...
    return Signal(meta, list(times), rows=lambda i0, i1: rows[i0:i1], ends=ends)
...
class TypeParDefautTest(unittest.TestCase):
    ...
    def test_sans_fins_timeseries(self):
        self.assertEqual(default_type(_signal()), DataType.TIMESERIES)

    def test_avec_fins_segments(self):
        s = _signal(times=(0.0, 10.0), ends=[5.0, 15.0],
                    rows=[{'startTimecode': 0.0}, {'startTimecode': 10.0}])
        self.assertEqual(default_type(s), DataType.SEGMENTS)
    ...
    def test_le_libelle_comments_n_est_PAS_consulte(self):
        ...
        self.assertEqual(default_type(s), DataType.TIMESERIES)
...
class Piege1_TempsDeSessionTest(unittest.TestCase):
    ...
    def test_l_offset_est_applique(self):
        f = frame_from_referential(self.ref, 'video')
        self.assertEqual(list(f.df['time']), [100.0, 101.0, 102.0])

    def test_deux_flux_d_offsets_differents_sont_alignes(self):
        a = frame_from_referential(self.ref, 'capteur')
        b = frame_from_referential(self.ref, 'video')
    ...
    def test_la_fenetre_est_lue_en_temps_de_SESSION(self):
        ...
        f = frame_from_referential(self.ref, 'video', t0=100.0, t1=101.0)
        self.assertEqual(list(f.df['time']), [100.0, 101.0])

    def test_le_signal_seul_ignore_l_offset_et_c_est_ASSUME(self):
        # `frame_from_signal` sur un flux du référentiel perdrait son décalage : c'est pourquoi
        # `frame_from_referential` est la porte d'entrée normale.
        f = frame_from_signal(self.ref.get('video'))
        self.assertEqual(list(f.df['time']), [0.0, 1.0, 2.0])

    def test_le_nom_du_referentiel_est_trace(self):
        self.assertEqual(frame_from_referential(self.ref, 'capteur').meta['referentiel'], 'essai')
...
class Piege2_ColonneTemporellePerimeeTest(unittest.TestCase):
    ...
    def test_le_temps_vient_des_times_PAS_de_la_ligne(self):
        ...
        rows = [{'timecode': 999.0, 'value': 1.0}, {'timecode': 998.0, 'value': 2.0}]
        s = _signal(times=(0.0, 0.5), rows=rows)
        f = frame_from_signal(s)
    ...
    def test_les_bornes_brutes_de_segment_sont_retirees_aussi(self):
        rows = [{'startTimecode': 0.0, 'endTimecode': 5.0, 'label': 'a'}]
        s = _signal(times=(0.0,), rows=rows, ends=[5.0])
        f = frame_from_signal(s)
...
class RetourVersSignalTest(unittest.TestCase):
    ...
    def test_le_lookup_suit_la_NATURE_pas_le_flux_d_origine(self):
        ...
        self.assertEqual(signal_from_frame(seg, 'a').meta.default_lookup, PREVIOUS)
        self.assertEqual(signal_from_frame(sig, 'b').meta.default_lookup, NEAREST)
...
class AdjoindreTest(unittest.TestCase):
    ...
    def test_le_flux_calcule_entre_au_referentiel(self):
        ...
        f = frame_from_referential(ref, 'brut')
        adjoin(ref, 'brut_derive', f)
...
class FenetreTest(unittest.TestCase):
    ...
    def test_la_fenetre_restreint_les_lignes(self):
        f = frame_from_signal(_signal(), t0=1.0, t1=2.0)
        self.assertEqual(list(f.df['time']), [1.0, 2.0])
        self.assertEqual(f.meta['fenetre'], (1.0, 2.0))

    def test_sans_fenetre_on_prend_tout(self):
        self.assertEqual(len(frame_from_signal(_signal()).df), 4)

    def test_restriction_de_champs(self):
        rows = [{'timecode': 0.0, 'a': 1, 'b': 2}]
        f = frame_from_signal(_signal(times=(0.0,), rows=rows), champs=['a'])
    ...
    def test_le_champ_canonique_survit_a_la_restriction(self):
        ...
        rows = [{'timecode': 0.0, 'a': 1}]
        f = frame_from_signal(_signal(times=(0.0,), rows=rows), champs=[])
        self.assertIn('time', f.df.columns)
...
class PontCompletTest(unittest.TestCase):
    ...
    def test_referentiel_vers_fonction_vers_referentiel(self):
        ...
        rows = [{'timecode': i * 0.5, 'value': float(i % 3)} for i in range(12)]
        ref.add(_signal('capteur', tuple(i * 0.5 for i in range(12)), rows=rows))

        cadre = frame_from_referential(ref, 'capteur')
        lisse = calc_rolling(cadre, window_s=1.0, column='value')
        self.assertIn('value_mean', lisse.df.columns)

        adjoin(ref, 'capteur_lisse', lisse)
    ...
    def test_referentiel_vers_SEGMENTATION_vers_referentiel(self):
        ...
        ref = TemporalReferential('session')
        rows = [{'timecode': float(i), 'value': 40.0 if 2 <= i <= 5 else 5.0}
                  for i in range(10)]
        ref.add(_signal('vitesse', tuple(float(i) for i in range(10)), rows=rows))

        cadre = frame_from_referential(ref, 'vitesse')
        segs = chain_to_segments(
            cadre, conditions=[{'key': 'C1', 'field': 'value',
                                'operator': '>=', 'value': 30.0}])
        self.assertEqual(len(segs.df), 1)

        adjoin(ref, 'survitesse', segs)
    ...
    def test_l_offset_survit_a_l_aller_retour(self):
        ...
        ref = TemporalReferential()
        rows = [{'timecode': float(i), 'value': float(i)} for i in range(5)]
        ref.add(_signal('decale', tuple(float(i) for i in range(5)), rows=rows),
                offset=1000.0)

        cadre = frame_from_referential(ref, 'decale')
        ...
        adjoin(ref, 'decale_derivee', derive)
