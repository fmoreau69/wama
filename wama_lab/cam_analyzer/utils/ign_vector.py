...
Pendant vectoriel d'`ortho_markings.py` (raster). **Règle posée 2026-07-28** : ce que l'IGN
publie en vecteur ne doit PAS être détecté visuellement. Les emprises de bâtiments et les
tronçons de route viennent d'ici — autoritatifs, exacts, avec leurs attributs (hauteur,
altitudes sol/toit) — tandis que les marquages au sol, absents de toute base vectorielle,
restent du ressort de la détection (SAM3 / détecteur open-vocab, cf. ROADMAP §17).
...
def _wfs_features(typename: str, lat: float, lon: float, radius_m: float, count: int = 1000):
    ...
    import requests
    from .ortho_markings import _proxies  # même besoin proxy UGE — ne pas dupliquer
    ...
    try:
        r = requests.get(_WFS, params=params, timeout=60, proxies=_proxies())
        r.raise_for_status()
