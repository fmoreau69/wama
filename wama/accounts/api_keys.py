...
portent une variable de clé. Ce module ne fait que dire, pour un utilisateur, quelles clés il a
posées et laquelle utiliser.
...
def llm_sources() -> list:
    """Les fournisseurs LLM qui demandent une clé, dans l'ordre de déclaration."""
    return [s for s in external_sources.SOURCES if s.kind == 'llm' and s.api_key_env]


def is_llm_source(key: str) -> bool:
    return any(s.key == key for s in llm_sources())
