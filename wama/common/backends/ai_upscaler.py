...
class AIUpscaler(BaseModelBackend):
    ...
    # ── Contrat commun (BaseModelBackend) — raccordé le 2026-09-06 ───────────────
    # Cette classe EST un moteur (onnxruntime), elle avait juste sa propre forme. Le
    # raccordement n'ajoute aucune logique : les quatre membres délèguent à ce qui existait.
    # Ce qu'il apporte : le moteur devient VISIBLE du registre, ses 7 modèles peuvent enfin
    # déclarer un moteur qui se résout, et le gouverneur de ressources voit sa VRAM —
    # l'upscaling n'était comptabilisé nulle part.
    #: Moteur piloté : c'est `onnxruntime` qui exécute (`ort.InferenceSession`), pas nous.
    ENGINE = 'onnxruntime'
    ...
    def load(self, model=None, **kwargs) -> bool:
        """`model` est ignoré : le modèle est choisi à la CONSTRUCTION (`model_name`), parce
        qu'il détermine l'échelle et le prétraitement. Le contrat tolère ce cas — c'est la
        même situation que DeepFace."""
        self._load_model()
    ...
    def process(self, image=None, **kwargs):
        """Point d'entrée du contrat — délègue au verbe métier historique."""
        return self.upscale_image(image, **kwargs)
