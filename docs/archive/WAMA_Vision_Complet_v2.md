...
> ---
> ### 🧭 À propos de cette v2 (2026-07-18)
>
> Version annotée de la vision : le texte original est **intégralement préservé** ; les ajouts
> sont marqués 🧭 et répondent aux points de l'[analyse critique](VISION_CRITIQUE.md).
> **État réel de chaque section** : voir [`VISION_STATUS.md`](VISION_STATUS.md) (✅/🔄/⏳) —
> convention de lecture : dans ce document, le présent décrit l'architecture VISÉE ; seul
> VISION_STATUS fait foi sur ce qui existe.
> **Marquage de maturité** utilisé dans les annotations : `[ÉTAGÈRE]` = intégration d'outils
> existants · `[INGÉNIERIE]` = construction maîtrisée · `[RECHERCHE]` = problème ouvert,
> à traiter comme axe de collaboration scientifique, pas comme promesse produit.
> ---

## 🧭 v2 — Non-objectifs et séquencement
...
>    des 10 apps génériques d'abord (c'est le goulot qui gate manifestes, génération d'apps et
>    prospection Phase B).
> 2. **Pas de RAG hiérarchique avant un RAG mono-niveau utile** (voir annotation §11).
> 3. **Pas de Story Director / storyboard / montage-mixage avant un Studio complet** (runners
>    pour toutes les apps, sorties gérées, batch orchestré) et sans cas d'usage labo nommé.
> 4. **Pas de SI laboratoire ni de médiathèque universitaire avant l'adoption interne** de la
>    médiathèque par l'équipe — et jamais sans le chapitre conformité (voir annotation §34).
> 5. **Pas de connecteurs conversationnels externes avant un assistant interne robuste** doté
>    d'un modèle de menace (voir annotation §48).
> 6. **Pas d'auto-instanciation d'apps sans revue humaine**, et seulement en dernier maillon de
>    la route manifeste (voir annotation §4).
> 7. **La couche Data (partie VII) attend la fondation RAG** et un premier cas d'usage labo réel.
>
> Toute exception à ces règles est une décision explicite, consignée, pas une dérive.

## 🧭 v2 — Parcours d'adoption (le point de vue du chercheur)

> La vision est écrite du point de vue du système ; l'adoption se joue du point de vue d'un
> chercheur non-technicien. Engagement de conception :
>
> - **5 minutes, zéro formation** : déposer un fichier → obtenir un résultat utile (transcription,
>   description, anonymisation). C'est le contrat du mode simplifié, déjà au cœur de l'UX
>   (cards, modales conservées, modes simplifié/avancé).
> - **Pente douce** : du dépôt simple → aux paramètres → aux batchs → au Studio. Chaque marche
>   est optionnelle ; personne n'a besoin du canvas pour bénéficier de WAMA.
> - **Rencontrer les usagers là où ils sont** (§48) plutôt que d'exiger une nouvelle habitude.
> - Chaque nouvelle capacité livrée doit répondre : « qu'est-ce que ça change pour un collègue
>   qui n'ouvrira jamais le Studio ? »
...
> 🧭 **v2 — Gating de l'auto-instanciation `[RECHERCHE→INGÉNIERIE]`.** Cette capacité est
> l'aboutissement de la route manifeste, pas son point de départ. Séquence verrouillée :
> uniformisation des apps → manifeste formel → conformité exécutable (`check_app_conformity`) →
> scaffold EN DERNIER, toujours avec revue humaine. L'écart mesuré entre « une IA analyse un
> dépôt » et un résultat fiable (campagne wama-dev-ai 2026-07-17 : affirmations d'absence fausses
> dans 4 rapports sur 6, sur des tâches étroites) justifie ce gating : on ne promet pas ici une
> trajectoire naturelle, on décrit un pari dont chaque maillon doit être validé.
...
> 🧭 **v2 — Marquage de maturité.** Le typage des flux média et le typage par connexion sont
> `[INGÉNIERIE]` (déjà en place dans le Studio). En revanche, « relier vidéo, EEG, trajectoires
> et événements dans un même espace de connaissance » par embeddings est `[RECHERCHE]` :
> l'alignement multimodal de données scientifiques hétérogènes est un problème ouvert. Le
> traiter comme un axe de collaboration (thèse, projet) et non comme une brique planifiée.
...
> 🧭 **v2 — Trajectoire incrémentale (remplace la lecture « 4 niveaux d'emblée »).** Le RAG est
> le point de dépendance de la moitié de cette vision (§9-10, 28-31, 34-39, 48-50) et n'existe
> pas encore. Ordre imposé : **(1)** RAG utilisateur minimal (ChromaDB + bge-m3, indexation via
> la médiathèque, branché sur l'étape `enrich` de la pipeline de prompts) ; **(2)** mesure d'usage
> réelle ; **(3)** extension aux niveaux équipe/labo/université **si et seulement si** la valeur
> du niveau de base est démontrée. La hiérarchie de gouvernance est une conséquence de l'usage,
> pas un préalable d'architecture.
...
> 🧭 **v2 — Ancrage de la partie V dans la mission du labo.** Les capacités créatives (§17-23)
> se justifient par des cas d'usage nommés : vidéos de restitution d'expérimentations, supports
> pédagogiques et MOOC, podcast/vulgarisation, illustrations de publications et de réponses à
> appels à projets. Chaque brique de cette partie doit pouvoir citer son client interne ; à
> défaut, elle relève de l'exploration opportuniste et passe après les non-objectifs (§ v2 en
> tête de document). Story Director, storyboard et montage/mixage (§21-23) restent gatés par un
> Studio complet.
...
> 🧭 **v2 — Maturité.** Fichiers de référence et modèles de contrôle : `[ÉTAGÈRE/INGÉNIERIE]`.
> Cohérence de personnages inter-scènes et direction artistique persistante : entre
> `[INGÉNIERIE]` et `[RECHERCHE]` selon le niveau d'exigence — prototyper avant de promettre.
...
> 🧭 **v2 — Garde-fous méthodologiques (s'applique à toute la partie VII, §27-33).** Une
> exploration automatisée de données scientifiques sans cadre statistique est une machine à
> artefacts (comparaisons multiples, corrélations fortuites, HARKing à l'échelle). Exigences :
> **(1)** distinction explicite exploratoire vs confirmatoire — toute « découverte » du système
> est étiquetée exploratoire tant qu'elle n'a pas été testée sur données indépendantes ;
> **(2)** correction des comparaisons multiples et tailles d'effet systématiques, pas seulement
> des p-values ; **(3)** l'hypothèse proposée par l'IA est validée par un chercheur AVANT
> d'entrer dans un pipeline confirmatoire ; **(4)** journal des analyses lancées (y compris
> infructueuses) pour rendre le taux de fausses découvertes estimable. C'est la condition de
> crédibilité du « DeepMind du labo » (§32-33) auprès du public visé. `[RECHERCHE]` pour la
> boucle de découverte ; `[INGÉNIERIE]` pour l'indexation et l'auto-description des jeux de
> données.
...
> 🧭 **v2 — Conformité et confiance (condition d'existence de la partie VIII).** Passer d'un
> outil de labo à une infrastructure institutionnelle change la nature du projet ; la barrière
> d'adoption sera réglementaire avant d'être technique. Prérequis non négociables :
> **RGPD** : base légale par traitement, registre, DPO associé dès la conception, DPIA pour les
> médias impliquant des personnes ; **droit à l'image et consentement** — en particulier pour
> tout enregistrement/transcription de réunions (§39 : information préalable, consentement,
> droits des instances représentatives) ; **anonymisation** non plus comme app optionnelle mais
> comme porte obligatoire avant tout partage élargi ou tout envoi cloud ; **authentification
> institutionnelle** (SSO) et traçabilité des accès ; **rétention** paramétrée par profil (déjà
> implémentée — à ériger en garantie affichée). La partie VIII se présente comme une offre de
> service avec coût d'exploitation et engagements, pas comme une extension logicielle.
...
> 🧭 **v2 — L'évaluation continue, couche de premier rang.** « Modèles interchangeables » (§47)
> n'est vrai que si l'on sait mesurer un remplacement. Exigences : chaque capacité déclarée dans
> un manifeste référence son **jeu de test interne** (fixtures du labo) et ses **métriques** ;
> la veille ne propose un remplacement qu'avec un **delta mesuré** sur ces jeux (jamais sur la
> seule réputation du modèle) ; les tests nocturnes tirent ces jeux (la charpente existe) et
> `qc.py` est câblé en post-génération. Sans cette couche, la promesse d'interchangeabilité
> reste un slogan.
...
> 🧭 **v2 — Modèle de réalisation et pérennité.** Cette vision est dimensionnée pour plusieurs
> équipes-années ; elle est portée par un développement assisté par IA autour d'un développeur
> principal (bus factor ≈ 1). Conséquences assumées : **(1)** la capitalisation par briques
> communes n'est pas qu'une élégance d'architecture, c'est LA stratégie de survie (moins de
> surface à maintenir) ; **(2)** chaque phase a un périmètre dimensionné à l'équipe réelle et
> des critères d'arrêt ; **(3)** la documentation vivante (conventions, manifestes, mémoire de
> projet) est ce qui rend le système reprenable par un tiers — elle fait partie du produit ;
> **(4)** toute dépendance opérationnelle critique (serveur, sauvegardes, modèles) doit être
> actionnable par au moins une seconde personne du labo.
...
> 🧭 **v2 — Modèle de menace de l'assistant connecté.** Un assistant joignable depuis des
> canaux (Mattermost, Matrix…) et capable de lancer des traitements est une surface d'attaque.
> Prérequis avant tout connecteur : **identité forte par canal** (liaison compte WAMA ↔ compte
> messagerie, pas de commande anonyme) ; **allowlist d'actions par rôle** — le canal conversationnel
> n'obtient jamais plus de droits que l'utilisateur authentifié ; **confirmation explicite** pour
> toute action destructive, coûteuse en GPU ou touchant aux données d'autrui ; **résistance à
> l'injection** : tout contenu tiers (messages, documents, sous-titres) est traité comme donnée,
> jamais comme instruction ; **journal d'audit** de toutes les actions déclenchées par
> conversation ; **quotas** par utilisateur et par canal. Cohérent avec la porte privacy
> avant-cloud (§52).
...
> 🧭 **v2 — Provenance : résolution de la tension abstraction ↔ science.** « L'utilisateur n'a
> pas à savoir quel modèle tourne » vaut pour le CONFORT, pas pour la PREUVE : une plateforme de
> recherche doit garantir la reproductibilité. Règle : l'orchestration choisit librement le
> modèle, MAIS chaque exécution journalise modèle + version + poids + paramètres + données
> d'entrée, et tout résultat peut être exporté en « fiche de méthode » citable (matériel &
> méthodes prêts pour publication). Les germes existent (états de run du Studio, statistiques
> d'exécution par modèle) — il s'agit de les ériger en garantie de premier rang. L'abstraction
> devient alors un argument scientifique au lieu d'une contradiction.
