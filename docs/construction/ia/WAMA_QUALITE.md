...
| ⑧ | **agrégation → indice** | par (tâche, modèle) : valeur, échelle, sens, population, version, date → `benchmark_meta` source `internal` | ⏳ | la MATIÈRE existe (`ResultEvaluation`, 2026-09-23 : chaque ligne porte déjà modèle, métrique + protocole, sens, référence = population) ; l'agrégation par modèle d'UN lot existe (`batch_evaluation`, taux de corpus) ; reste l'agrégat INTER-lots vers `benchmark_meta` (Q8) — demande explicite de Fabien : « réutiliser cette évaluation comme note de chaque modèle » ; règle de lot déjà écrite dans `_quality_scalars` |
