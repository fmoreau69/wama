...
- **Adoption réelle — RE-MESURÉE le 2026-08-31** (balayage exhaustif des 10 apps, fichier:ligne
  ...
  | **cycle complet** de modale item (`WamaParams.settingsModal`) | ~~2/10~~ **8/10 au 2026-09-24** (anonymizer, imager, enhancer 21/09, reader · synthesizer · composer · describer · avatarizer 24/09) | 2 apps réécrivent encore ouvrir→rendre→sauver→fermer à la main : **converter** (pas un quick win : sa modale charge les valeurs par `status`, patche l'aide moteur par type, désactive Appliquer sur RUNNING, et ses boutons « Sauver comme profil » / « Par défaut » lisent le corps de la modale — à porter avec `fetchUrl`, `decorate` et un pied à `left_html`, en une passe dédiée) et transcriber (autre session). **Avatarizer** : sa modale câble `WamaModelCaps` + `WamaInputMatch` à chaque ouverture (`decorate`) ; préalable levé À LA BRIQUE — l'écouteur ✕ de `WamaInputMatch` est unique pour la page (il en posait un par `init`) ; geste d'appariement mesuré sur une modale ROUVERTE, 10/10 (`INPUT_MODEL_MATCHING §6.7`). ⚠ Ne pas confondre les deux lignes : c'est la confusion déclencheur/cycle qui a rendu toutes les tables précédentes fausses |
...
- **à faire maintenant** : ~~`card_refresh_common` (10 apps)~~ (10/10 le 22/09), `settings_modal_cycle` (~~8~~ 2 au 24/09), rangée
