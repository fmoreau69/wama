...
- **Adoption réelle — RE-MESURÉE le 2026-08-31** (balayage exhaustif des 10 apps, fichier:ligne
  ...
  | **cycle complet** de modale item (`WamaParams.settingsModal`) | ~~2/10~~ **9/10 au 2026-09-24** (anonymizer, imager, enhancer 21/09, reader · synthesizer · composer · describer · avatarizer · converter 24/09) | 1 app réécrit encore ouvrir→rendre→sauver→fermer à la main : transcriber (autre session). **Converter** : valeurs lues par `status` avant ouverture, `collect` → `output_format` + `options_json` par le lecteur schéma, `decorate` → garde RUNNING, relance, « Par défaut », « Sauver comme profil » (par classe). ⚠ Le geste `<app>.settings` a été ÉTENDU (fidélité au ⚙, réouverture, actions du pied — `WAMA_VERIFICATION §3` geste 2) et a trouvé au 1er passage une régression de la modale générée : valeur enregistrée perdue au remplissage asynchrone des options — corrigée À LA BRIQUE (`wama-params.js`). **Avatarizer** : sa modale câble `WamaModelCaps` + `WamaInputMatch` à chaque ouverture (`decorate`) ; préalable levé À LA BRIQUE — l'écouteur ✕ de `WamaInputMatch` est unique pour la page (il en posait un par `init`) ; geste d'appariement mesuré sur une modale ROUVERTE, 10/10 (`INPUT_MODEL_MATCHING §6.7`). ⚠ Ne pas confondre les deux lignes : c'est la confusion déclencheur/cycle qui a rendu toutes les tables précédentes fausses |
...
- **S. BAC À SABLE — jumelle EXÉCUTABLE (actée Fabien 2026-08-18, marche suivante).** Constat :
  ...
  > | ⚙ sans effet sur `describer_01` (24/09 — « aucune app n'a déclaré d'ouvreur », mesuré au navigateur ; la vue d'édition était un bouchon 501) | la route d'édition d'élément a QUATRE noms dans le parc (`update`, `update_options`, `update_settings`, `save_settings`), le générateur n'en connaissait que deux ; et `update_settings` est AUSSI la route GLOBALE de l'anonymizer, sans pk | alias ajoutés + `urls_gen.ITEM_ROUTES`/`alias_fits` : un alias de route d'élément n'est retenu que si son motif déclaré prend `<int:pk>` — lu par le gabarit (ouvreur) ET les vues (corps) ; `tests_codegen_lot.ItemEditRouteAliasTest` sur les 10 apps + contre-épreuve anonymizer ; geste `describer_01.settings` vert après régénération |
  > | « nature '' sans backend déclaré » (describer_01 24/09, geste `processing`) | DEUX gabarits DEVINAIENT le même fait : `tasks_gen` lisait la colonne déclarée (`backend_nature_field` = `detected_type`), `views_gen` supposait `media_type` — l'upload n'écrivait jamais ce que la tâche lisait | `views_gen` lit la MÊME déclaration ; le lot ne reçoit la nature que s'il porte la colonne ; `NatureFieldAgreementTest` (10 apps) |
  > | SUCCESS puis téléchargement 501 (describer_01 24/09) | le gabarit ne servait que `output_file` ; le résultat DÉCLARÉ était un texte (`backend_result` = `result_text`) | `download`/`download_all` servent le résultat déclaré (fichier → sa colonne, texte → `.txt` par `compose_output_name`) ; brique `make_batch_views(output_text=…)` ; `DeclaredResultDownloadTest`, `TextResultBatchDownloadTest` |
  > | Toute substitution revertée après un RENOMMAGE de route à la source (converter_01 24/09, `update_job` → `update_settings`) | `urls` généré importe une vue que les vues de la jumelle n'ont pas, `views` généré retire le nom que ses urls importent, les templates générés citent l'ancien nom ; et le revert ramène au TÉMOIN d'ORIGINE, pas à l'état généré précédent | `app_sandbox substitute <jumelle> urls+views+templates` : plusieurs cibles, UNE substitution (générées, mesurées, revertées ensemble) ; `CoupleViewsTemplatesTest` (+2) |
  >
  ...
  > à modèle de LIAISON — garde SES templates copiés, c'est cohérent et déclaré). Quand la
  > SOURCE a renommé une route, les passer ENSEMBLE : `urls+views+templates`.
  ...
  **S2 — JALON (18/08, RECALÉ à l'audit du 31/08) : 7/7 substitutions TIENNENT** — apps,
  ...
  la forme à modèle de liaison est un trou déclaré.~~ ✅ **Les DEUX formes sont rendues depuis
  le 2026-09-22** : la liaison est LUE au manifeste (`processing.model_spec.batch` —
  `link_name`, `link_item_field`, `link_batch_field`, `link_batch_related`, déclarée depuis
  l'extract, jamais devinée sur un nom de classe) et les vues générées appellent les briques
  des apps réelles (`make_queue_manipulation_views`, `build_batches_list`, `auto_wrap_orphans`,
  `wrap_in_batch`, et — les deux formes — `batch_elements` pour LIRE un lot dans l'ordre des
  lignes, `attach_to_batch` pour y RATTACHER). Le fichier généré ne porte de sa forme que les
  kwargs déclarés, dans un seul wrapper `_link_to_batch` ; un seul corps par vue de lot.
  ⚠ Remarque de Fabien le jour même, à retenir : ma première version émettait un helper généré
  `_batch_elements` à côté de la brique `elements_du_lot` (08/09, nommée en français) — un
  chemin PARALLÈLE. Retiré : la brique est renommée `batch_elements`, garantit l'ordre des
  lignes et charge l'élément en une requête ; `attach_to_batch` (nouvelle, extraite de
  `wrap_in_batch`) sert AUSSI les deux fabriques de manipulation de file, qui recopiaient
  trois fois le même `objects.create`. Le rendu de la forme directe n'en est changé que de
  quelques lignes (diff contre l'ancien générateur). Les 10 apps
  rendent un `views.py` qui compile (`tests_codegen_lot.LinkFormViewsTest`). Deux défauts
  ANTÉRIEURS du générateur, révélés en le faisant tourner sur le parc : une vue de CLASSE en
  extra (`views.ProcessView.as_view()`, anonymizer) produisait `def as_view()` — bouchon de
  classe désormais ; et `_decorer` posait `gear_data` sur un modèle COPIÉ dont c'est une
  @property sans setter (describer_01 : page vide 200, page habitée 500) — `_set_unless_property`
  laisse parler la glu du modèle réel tant que `models` n'est pas substitué.
...
- **à faire maintenant** : ~~`card_refresh_common` (10 apps)~~ (10/10 le 22/09), `settings_modal_cycle` (~~8~~ 1 au 24/09 : transcriber), rangée
