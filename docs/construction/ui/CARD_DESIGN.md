...
| 8 | **Détection batch** : extension (`txt/md/csv`) → garde MIME → **arbitrage serveur par le CONTENU** (`count=0` ⇒ fichier de travail) ; 1 seul fichier peut être descripteur ; refus ligne-à-ligne visible | ~~ids FIXES ⇒ **une seule barre par page** (d'où le clone `_audio_batch_bar.html` de l'enhancer, hors brique)~~ ✅ **résorbé le 2026-09-08** : `WamaBatchImport({idBase})` + `batch_detect_bar.html` paramétré (`bid`, `label`) — la voie audio de l'enhancer inclut la barre commune, le clone a disparu ; l'URL saisie passe par le MÊME pipeline (`ingestText`) dans les 10 apps | `wama/common/static/common/js/batch-import.js` ; `wama/common/templates/common/batch_detect_bar.html` |
...
`batch_detect_bar.html` — qui n'est plus mono-instance depuis le 2026-09-08 (`bid`/`idBase`,
défaut n°8 résorbé) : la v4 peut le rendre par card sans le cloner.
